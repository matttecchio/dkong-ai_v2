...
model=$(ls -t artifacts/ppo_dkong_run27_last.zip artifacts/checkpoints/ppo_dkong_run27/*.zip 2>/dev/null | head -1)
if [ -z "$model" ]; then
    echo "$ts: NO MODEL FOUND — aborting"
    exit 1
fi

log="logs/run27_auto_${ts}.log"
pid=$(scripts/current_launch.sh "$model" "$log")
echo "$pid" > logs/run_current.pid
ln -sfn "run27_auto_${ts}.log" logs/run_current.log
date +%s >> "$journal"
