...
#grid { display:grid; grid-template-columns:repeat(2, max-content); gap:14px; }
.cell { position:relative; }
.cell img.bg { display:block; width:400px; image-rendering:pixelated; }
...
<div id=bar>
  <h1>DK LIVE &middot; 16 ENVS &middot; 4 BOARDS</h1>
...
for(let i=0;i<4;i++){
  ...
  cell.innerHTML='<img class=bg src="'+BG+'">'
    ...
    +'<div class=tag>ENVS '+(5000+i*4)+'-'+(5003+i*4)+'</div>';
...
async function tick(){
  try{
    ...
    let live=0;
    for(const s of st){
      const sv=panels[Math.min(3,(s.port-5000)>>2)];
      if(!marks[s.port]){
        trails[s.port]=el(sv,'polyline',{fill:'none','stroke-width':1.5,opacity:.45});
        rings[s.port]=el(sv,'ellipse',{rx:18,ry:5,fill:'none','stroke-width':2.5});
        marks[s.port]=el(sv,'image',{href:MARIO,width:40,height:56});
        threats[s.port]=[];
      }
      const m=marks[s.port], ring=rings[s.port], tr=trails[s.port], th=threats[s.port];
      ...
      while(th.length<objs.length)th.push(el(sv,'image',{opacity:0}));
      th.forEach((e,i)=>{
